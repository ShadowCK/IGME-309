...
void MyCamera::MoveForward(float a_fDistance)
{
	...
	// Update position, target and above moved in forward direction by a_fDistance
...
void MyCamera::MoveVertical(float a_fDistance)
{
	...
	// Update position, target and above moved in global upward (Y axis) by a_fDistance
...
{
	//Tip:: Look at MoveForward
	// Update position, target and above moved in right direction by a_fDistance
...
void MyCamera::CalculateView(void)
{
	...
	// Forward (camera orientation) : target - position
	m_v3Forward = m_v3Target - m_v3Position;
	// Right : cross product of forward and global upward (y axis)
	m_v3Rightward = glm::normalize(glm::cross(m_v3Forward, AXIS_Y));
	// Upward : cross product of right and forward
	m_v3Upward = glm::normalize(glm::cross(m_v3Rightward, m_v3Forward));

	// Calculate current pitch
	float currentPitch = atan2(m_v3Forward.y, glm::length(vector2(m_v3Forward.x, m_v3Forward.z)));
	// Pitch is constrained within [-89, 89] degrees
	if ((currentPitch <= glm::radians(89.0f) || m_v3PitchYawRoll.x <= 0)
		&& (currentPitch >= glm::radians(-89.0f) || m_v3PitchYawRoll.x >= 0)) {
		// * Apply pitch quaternion to forward
		quaternion pitchQuat = glm::angleAxis(m_v3PitchYawRoll.x, m_v3Rightward);
		m_v3Forward = pitchQuat * m_v3Forward;
	}

	// * Apply yaw quaternion to forward
	quaternion yawQuat = glm::angleAxis(m_v3PitchYawRoll.y, m_v3Upward);
	m_v3Forward = yawQuat * m_v3Forward;

	// Update the target vector
	m_v3Target = m_v3Forward + m_v3Position;
	// Update the view matrix
	m_m4View = glm::lookAt(m_v3Position, m_v3Target, m_v3Upward);
	// Reset the pitch, yaw, roll vector
