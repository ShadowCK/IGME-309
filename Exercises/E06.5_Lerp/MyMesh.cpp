...
{
	GenerateCircle(a_fRadius, a_nSubdivisions, ZERO_V3, a_v3Color);
}
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_offset, vector3 a_v3Color)
{
	Release();
	Init();
	...
	for (int i = 0; i < a_nSubdivisions; i++) {
		...
		float x1 = cos(2 * PI * i / a_nSubdivisions) * a_fRadius;
		float y1 = sin(2 * PI * i / a_nSubdivisions) * a_fRadius;

		float x2 = cos(2 * PI * (i + 1) / a_nSubdivisions) * a_fRadius;
		float y2 = sin(2 * PI * (i + 1) / a_nSubdivisions) * a_fRadius;

		AddTri(vector3(x1, y1, z) + a_offset, vector3(x2, y2, z) + a_offset, center + a_offset);
	}
	/*
